...
`}function Wt(){let e=s.createShaderModule({code:yn()});je=s.createComputePipeline({layout:"auto",compute:{module:e,entryPoint:"main"}}),pe?.destroy(),pe=s.createBuffer({size:Vt,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),Pr=s.createBindGroup({layout:je.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:v}},{binding:1,resource:{buffer:pe}}]}),xr=s.createBindGroup({layout:je.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:R}},{binding:1,resource:{buffer:pe}}]})}function Cn(e,t,r,n,i,o,c){let l=te.get(Ie)??0,d=Qr++,f=new ArrayBuffer(Vt),u=new Int32Array(f),a=new Uint32Array(f);u[0]=t,u[1]=r,a[2]=S,a[3]=_,a[4]=n,a[5]=i,a[6]=o,a[7]=l,a[8]=d,a[9]=c.length,a[10]=0;for(let U=0;U<c.length&&U<32;U++)a[11+U]=c[U];s.queue.writeBuffer(pe,0,f);let g=Math.ceil(n/8),T=e.beginComputePass();T.setPipeline(je),T.setBindGroup(0,A?xr:Pr),T.dispatchWorkgroups(g,g),T.end()}function _n(){let e=A?R:v,t=pt(),r;try{r=s.createBuffer({size:t,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST})}catch(i){return console.warn("GPU readback buffer allocation failed:",i),Promise.reject(new Error(`Failed to allocate ${t} byte readback buffer`))}let n=s.createCommandEncoder();return n.copyBufferToBuffer(e,0,r,0,t),s.queue.submit([n.finish()]),r.mapAsync(GPUMapMode.READ).then(()=>{let i=new Uint32Array(r.getMappedRange().slice(0));return r.unmap(),r.destroy(),i})}function Gr(){if(m=pt(),!ne()){C=0;return}let e=Sn();C=Math.max(1,Math.floor(e/m))}function Sn(){return m>=or?m:Math.min(Math.max(or,m),Xt())}function Ur(){if(C<1||m<=0)return ir;let e=Math.max(m,C*m),t=Math.floor(en/e);return Math.max(1,Math.min(ir,t||1))}function Kt(){let e=ne();self.postMessage({type:"limits",maxBytes:Se(),vramBudgetBytes:Fr(),frameByteSize:m,recordingAvailable:e,vramSimulationBytes:sn(),vramRecordingBytes:on(),gridFormat:se()})}function ee(){return!ne()||C<1||G===null||J.length===0||ue>=Ur()?!1:b<C?!0:J.some((e,t)=>z[t]&&e.mapState==="unmapped")}function Z(e){if(C<1||G===null||b>=C)return;let t=A?R:v,r=b*m,n=s.createCommandEncoder();n.copyBufferToBuffer(t,0,G,r,m),s.queue.submit([n.finish()]),x.push(e),b++}function V(){if(G===null||b===0||J.length===0)return;let e=z.indexOf(!0);if(e<0)return;z[e]=!1;let t=J[e];if(t.mapState!=="unmapped"){z[e]=!0;return}let r=b*m,n=Mr++,i=[...x],o=i[0],c=i[i.length-1],l=`chunk-${String(n).padStart(6,"0")}.bin`,d=b,f=s.createCommandEncoder();f.copyBufferToBuffer(G,0,t,0,r),s.queue.submit([f.finish()]);let u={chunkId:n,generationStart:o,generationEnd:c,blockCount:d,codec:dt,uncompressedBytes:r,storedBytes:r,gridFormat:se(),generations:i,filename:l};Et(1),ue++,ge();let a=ke;t.mapAsync(GPUMapMode.READ).then(async()=>{let g=t.getMappedRange(),T=new ArrayBuffer(r);new Uint8Array(T).set(new Uint8Array(g,0,r)),t.unmap(),a===ke&&(z[e]=!0,ge(),y.push(u),Qt(),Bn(u,T).then(()=>{a===ke&&(ue--,ge(),Et(-1),me(),self.postMessage({type:"chunkSealed",filename:u.filename,rawBytes:r,blockCount:u.blockCount,cols:S,rows:_,rawGridFormat:u.gridFormat,storageGridFormat:Pe(yt(Re.tribes.length))}),Fe&&Q===0&&(Fe=!1,Or()))}))}).catch(()=>{a===ke&&(z[e]=!0,ue--,ge(),Et(-1))}),b=0,x=[]}function Qt(){y.length>0&&(H.generationStart=y[0].generationStart,H.generationEnd=y[y.length-1].generationEnd),x.length>0&&(y.length===0&&(H.generationStart=x[0]),H.generationEnd=x[x.length-1]),H.chunks=[...y]}async function dr(e){ke++,Mr=0,b=0,x=[],y=[],ue=0,Q>0&&(Q=0,self.postMessage({type:"chunksSaving",active:!1})),P&&(P=!1,self.postMessage({type:"backpressure",active:!1})),Fe=!1,H={chunks:[],generationStart:e,generationEnd:e,gridFormat:se()},await Dr(),me()}async function Jt(){return ie&&await ie,Me||(Me=await(await navigator.storage.getDirectory()).getDirectoryHandle(tt,{create:!0})),Me}async function Bn(e,t){let i=await(await(await Jt()).getFileHandle(e.filename,{create:!0})).createWritable();await i.write(t),await i.close()}async function En(e){let t=await Jt();for(let r of e)try{await t.removeEntry(r)}catch(n){console.warn(`Failed to remove OPFS entry ${r}:`,n)}}async function Dr(){if(ie){await ie;return}ie=(async()=>{let e=await navigator.storage.getDirectory();Me=null;try{await e.removeEntry(tt,{recursive:!0})}catch(t){t instanceof DOMException&&t.name==="NotFoundError"||console.warn(`Failed to remove OPFS directory ${tt}:`,t)}Me=await e.getDirectoryHandle(tt,{create:!0})})();try{await ie}finally{ie=null}}function Or(){Qt(),self.postMessage({type:"recording",manifest:{chunks:y.map(e=>({...e,generations:[...e.generations]})),generationStart:H.generationStart,generationEnd:H.generationEnd,gridFormat:se()},cols:S,rows:_})}function lt(){return b>0?x[b-1]!==h:y.length>0?y[y.length-1].generationEnd!==h:!0}function pr(){!B||!lt()||!ee()||(b>=C&&V(),Z(h))}function Nr(){if(!Ze)return;let e=Ze;Ze=null;let t=s.createCommandEncoder();Cn(t,e.centerX,e.centerY,e.brushSize,e.shape,e.fill,e.tribeIds),s.queue.submit([t.finish()]),B&&b>0&&x[b-1]===h&&(b--,x.pop(),Z(h))}async function Pn(e,t=dt){let o=await(await(await(await Jt()).getFileHandle(e)).getFile()).arrayBuffer();return t===Rr?an(o):o}function gr(){let e=b;for(let t of y)e+=t.blockCount;return e}function W(e){let t=Math.ceil(S/16),r=Math.ceil(_/16),n=new Uint32Array(256);s.queue.writeBuffer(be,0,n);let i=e.beginComputePass();i.setPipeline(Ve),i.setBindGroup(0,A?kr:Tr),i.dispatchWorkgroups(t,r),i.end(),e.copyBufferToBuffer(be,0,he,0,256*4);let o=new Uint32Array([0]);s.queue.writeBuffer(ye,0,o);let c=e.beginComputePass();c.setPipeline(Qe),c.setBindGroup(0,A?Ar:wr),c.dispatchWorkgroups(t,r),c.end(),e.copyBufferToBuffer(ye,0,Ce,0,4)}function K(){let e=h;if(e===$||M)return;$=e,M=!0;let t=[];t.push(he.mapAsync(GPUMapMode.READ)),t.push(Ce.mapAsync(GPUMapMode.READ)),Promise.all(t).then(()=>{let r=te.get(Ie)??0,n={},i=0,o=0,c={},l=new Uint32Array(he.getMappedRange().slice(0));he.unmap();let d=0;for(let a=0;a<w.length;a++){let g=l[a]??0;n[w[a].id]=g,a!==r&&(d+=g,g>0&&(Tt.set(a,e),kt.add(a)))}if(d>0)for(let a=0;a<w.length;a++){if(a===r)continue;let g=(l[a]??0)/d;g>0&&(i-=g*Math.log2(g),o+=g*g)}for(let a=0;a<w.length;a++){if(a===r)continue;(l[a]??0)>0?c[w[a].id]=null:kt.has(a)?c[w[a].id]=Tt.get(a)??0:c[w[a].id]=0}let f=new Uint32Array(Ce.getMappedRange().slice(0));Ce.unmap();let u=f[0]??0;if(M=!1,self.postMessage({type:"metrics",generation:e,population:n,shannonEntropy:i,simpsonIndex:1-o,boundaryLength:u,extinctionTime:c,totalFrames:gr(),fps:jt,canStepBack:gr()>1,recordingBytes:y.reduce((a,g)=>a+g.storedBytes,0),recordingRawBytes:y.reduce((a,g)=>a+g.uncompressedBytes,0)}),q){q=!1,$=-1;let a=s.createCommandEncoder();W(a),s.queue.submit([a.finish()]),K()}}).catch(()=>{M=!1})}function mr(){let e=S*_;return e>1e7?10:e>1e6?50:e>1e5?200:1e3}function xn(){let e=S*_;return e>1e7?2:e>1e6?4:e>1e5?8:16}function br(e){if(e<=0)return;let t=Math.ceil(ct/16),r=Math.ceil(_/16),n=s.createCommandEncoder();for(let i=0;i<e;i++){let o=n.beginComputePass();o.setPipeline(Ae),o.setBindGroup(0,A?qt:zt),o.dispatchWorkgroups(t,r),o.end(),A=!A,h++}s.queue.submit([n.finish()]),oe+=e}function Xe(){self.postMessage({type:"generation",generation:h,fps:jt})}function st(){let e=s.createCommandEncoder(),t=e.beginComputePass();t.setPipeline(Ae),t.setBindGroup(0,A?qt:zt);let r=Math.ceil(ct/16),n=Math.ceil(_/16);t.dispatchWorkgroups(r,n),t.end(),s.queue.submit([e.finish()]),A=!A,h++}function ae(){gn();let e=Pt.getCurrentTexture().createView(),t=s.createCommandEncoder(),r=t.beginRenderPass({colorAttachments:[{view:e,loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:1}}]});r.setPipeline(it),r.setBindGroup(0,A?_r:Cr),r.draw(3),r.end(),s.queue.submit([t.finish()])}function Y(e){if(re||le){self.requestAnimationFrame(Y);return}He===0&&(He=e);let t=e-He;if(t>=1e3&&(jt=oe/(t/1e3),oe=0,He=e),D>=0){if(B){let n=!1,i=performance.now()+14;for(;h<D&&performance.now()<i;){if(!ee()){n=!0;break}b>=C&&V(),st(),oe++,Z(h)}if(n){P||(P=!0,self.postMessage({type:"backpressure",active:!0})),e-O>=1e3&&(O=e,Xe()),s.queue.onSubmittedWorkDone().then(()=>{self.requestAnimationFrame(Y)});return}P&&(P=!1,self.postMessage({type:"backpressure",active:!1}))}else{let n=Math.min(mr(),D-h);br(n)}if(e-O>=1e3&&(O=e,Xe()),h>=D){if(D=-1,I=wt,k=Je,j=0,X=0,O=0,P&&(P=!1,self.postMessage({type:"backpressure",active:!1})),$=-1,M)q=!0;else{let n=s.createCommandEncoder();W(n),s.queue.submit([n.finish()]),K()}ae(),self.postMessage({type:"stepping",active:!1}),self.requestAnimationFrame(Y)}else B?self.requestAnimationFrame(Y):s.queue.onSubmittedWorkDone().then(()=>{self.requestAnimationFrame(Y)});return}Nr();let r=!1;if(I){B&&lt()&&ee()&&(b>=C&&V(),Z(h));let n=!1;j===0&&(j=e);let i=e-j;if(j=e,k<=0){if(B){let o=!1,c=performance.now()+14;for(;performance.now()<c;){if(!ee()){o=!0;break}b>=C&&V(),st(),oe++,n=!0,Z(h)}if(o){if(P||(P=!0,self.postMessage({type:"backpressure",active:!0})),e-O>=1e3&&(O=e,Xe()),n&&(e-fe>=1e3||fe===0)&&!M){fe=e;let d=s.createCommandEncoder();W(d),s.queue.submit([d.finish()]),K()}s.queue.onSubmittedWorkDone().then(()=>{self.requestAnimationFrame(Y)});return}P&&(P=!1,self.postMessage({type:"backpressure",active:!1}))}else if(!_e){let o=mr(),c=xn();for(let l=0;l<c;l++)br(o),n=!0;_e=!0,s.queue.onSubmittedWorkDone().then(()=>{_e=!1,vr()?Mt():et()})}e-O>=1e3&&(O=e,Xe())}else for(X+=i;X>=k;){if(B){if(!ee())break;b>=C&&V()}st(),oe++,X-=k,n=!0,B&&Z(h)}n&&(r=(e-fe>=1e3||fe===0)&&!M)}if(k>0&&!At&&ae(),r){fe=e;let n=s.createCommandEncoder();W(n),s.queue.submit([n.finish()]),K()}k<=0&&!B&&I||self.requestAnimationFrame(Y)}function Tn(e,t){let r=s?Se():Number.POSITIVE_INFINITY;return rr(t.bitsPerCell)&&bt(t.bitsPerCell,e.tribes.length)&&ht(e.cols,e.rows,Ee(t.bitsPerCell),r)?Ee(t.bitsPerCell):nr(e.tribes.length,e.cols,e.rows,r)}function hr(e,t){Re=e,S=e.cols,_=e.rows,p=Tn(e,t),ct=xe(S,p),w=[...e.tribes],H.gridFormat=se(),te.clear(),w.forEach((r,n)=>te.set(r.id,n))}async function $r(e){ce=e;let t=await navigator.gpu.requestAdapter();if(!t)throw new Error("WebGPU adapter not available");s=await t.requestDevice({requiredLimits:{maxBufferSize:t.limits.maxBufferSize,maxStorageBufferBindingSize:t.limits.maxStorageBufferBindingSize}}),le=!1,s.lost.then(n=>{let i=n.message||n.reason||"unknown";le=!0,I=!1,re=!0,self.postMessage({type:"deviceLost",reason:i})}),self.postMessage({type:"limits",maxBytes:Se(),vramBudgetBytes:Fr(),frameByteSize:0,recordingAvailable:!0,vramSimulationBytes:0,vramRecordingBytes:0,gridFormat:se()});let r=ce.getContext("webgpu");if(!r)throw new Error("WebGPU canvas context not available");Pt=r,xt=navigator.gpu.getPreferredCanvasFormat(),Pt.configure({device:s,format:xt,alphaMode:"opaque"})}async function kn(){try{return await $r(ce),!0}catch(e){let t=e instanceof Error?e.message:String(e);return le=!0,I=!1,re=!0,self.postMessage({type:"deviceLost",reason:t}),!1}}async function Wr(){G=s.createBuffer({size:C*m,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC}),await at(C*m,G),b=0,x=[]}async function Kr(){let e=C*m;J=[],z=[];for(let t=0;t<ft;t++){let r=s.createBuffer({size:e,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});J.push(r),z.push(!0),await at(e,r)}}async function wn(){await Dr()}async function An(){It(),Gr(),await Gt(),Ut(),Dt(),Ot(),Nt(),Wt(),$t(),await wn(),ne()?(await Wr(),await Kr()):(ut(),B=!1),await ot(),Kt()}async function Mn(){re=!0,self.postMessage({type:"rebuilding",active:!0});try{await Ht()}catch{}if(le&&!await kn())return!1;lr(),It(),Gr(),ur(ne());try{await Gt(),Ut(),Dt(),Nt(),Wt(),Ot(),$t(),ne()?(await Wr(),await Kr()):(ut(),B=!1),await ot(),Kt()}catch(e){let t=e instanceof Error?e.message:String(e);self.postMessage({type:"gpuError",reason:t});try{lr(),It(),ur(!1),await Gt(),Ut(),Dt(),Nt(),Wt(),Ot(),$t(),B=!1,m=pt(),ut(),await ot(),Kt()}catch(r){return console.warn("GPU recovery also failed, device may be lost:",r),!1}}return re=!1,self.postMessage({type:"rebuilding",active:!1}),!0}self.onmessage=async e=>{let t=e.data;switch(t.type){case"init":{B=t.recording,hr(t.ruleset,t.simulationGridFormat),await $r(t.canvas),await An(),me(),I=t.running,k=t.speed<0?0:1e3/t.speed,j=0,X=0,self.requestAnimationFrame(Y);break}case"setRuleset":{if(hr(t.ruleset,t.simulationGridFormat),!await Mn())break;if(h=0,$=-1,await dr(0),Tt=new Map,kt=new Set,M)q=!0;else{let n=s.createCommandEncoder();W(n),s.queue.submit([n.finish()]),K()}break}case"setRunning":if(!t.running&&D>=0){if(D=-1,I=!1,k=Je,j=0,X=0,P&&ge(),$=-1,M)q=!0;else{let r=s.createCommandEncoder();W(r),s.queue.submit([r.finish()]),K()}ae(),self.postMessage({type:"stepping",active:!1});break}if(I=t.running,t.running)j=0,X=0,Mt();else{if(P&&ge(),$=-1,M)q=!0;else{let r=s.createCommandEncoder();W(r),s.queue.submit([r.finish()]),K()}k<=0&&!B&&D<0&&!_e&&et()}break;case"setSpeed":{let r=k<=0,n=t.speed<0?0:1e3/t.speed;r&&n>0&&(At=!0,s.queue.onSubmittedWorkDone().then(()=>{At=!1,ae(),et()})),k=n,X=0,O=0,!r&&n<=0?Mt():r&&n>0&&!_e&&et();break}case"camera":Sr=t.scale,Br=t.offsetX,Er=t.offsetY;break;case"resize":ce.width=t.width,ce.height=t.height;break;case"draw":{let r=t.tribes.map(n=>te.get(n)).filter(n=>n!==void 0);if(r.length>0){let n={square:0,round:1,diamond:2,vline:3,hline:4},i={full:0,spray:1,outline:2};Ze={centerX:t.x,centerY:t.y,brushSize:t.size,shape:n[t.shape]??0,fill:i[t.fill]??0,tribeIds:r}}break}case"getSnapshot":{_n().then(r=>{self.postMessage({type:"snapshot",grid:r,generation:h,cols:S,rows:_,gridFormat:se()},[r.buffer])}).catch(()=>{let r=new Uint32Array(0);self.postMessage({type:"snapshot",grid:r,generation:h,cols:S,rows:_,gridFormat:se()})});break}case"loadSnapshot":{let r=A?R:v,n=Ct(t.gridFormat),i=Te(S,_,n);if(t.grid.byteLength!==i)break;let o=n.bitsPerCell===p.bitsPerCell?t.grid:_t(St(t.grid,S,_,n),S,_,p);s.queue.writeBuffer(r,0,o),h=t.generation,await dr(t.generation);break}case"setRecording":{if(t.recording&&ne()&&!B){if(B=!0,pr(),$=-1,M)q=!0;else{let r=s.createCommandEncoder();W(r),s.queue.submit([r.finish()]),K()}me()}else(!t.recording||!ne())&&(B=!1);break}case"getRecording":{if(Fe)break;await Ht(),pr(),b>0&&V(),Q>0?Fe=!0:Or();break}case"stepBack":{let r=0;for(let l of y)r+=l.blockCount;let n=r+b,i=Math.min(t.count,n-1);if(i<=0)break;let o=n-1-i,c=A?R:v;if(o>=r){let l=o-r;b=l+1,x.length=b,h=x[l];let d=s.createCommandEncoder();d.copyBufferToBuffer(G,l*m,c,0,m),s.queue.submit([d.finish()])}else{if(Q>0){await new Promise(E=>{let L=setInterval(()=>{Q===0&&(clearInterval(L),E())},10)}),r=0;for(let E of y)r+=E.blockCount}let l=0,d=0,f=0;for(let E=0;E<y.length;E++){let L=y[E];if(o<l+L.blockCount){d=E,f=o-l;break}l+=L.blockCount}let u=y[d],a=await Pn(u.filename,u.codec),g=Ct(u.gridFormat),T=Te(S,_,g);if(g.bitsPerCell===p.bitsPerCell){let E=(f+1)*m;s.queue.writeBuffer(G,0,new Uint8Array(a,0,E))}else{let E=new Uint8Array((f+1)*m);for(let L=0;L<=f;L++){let zr=L*T,qr=new Uint8Array(a,zr,T),Yr=sr(qr,S,_,g),gt=_t(Yr,S,_,p);E.set(new Uint8Array(gt.buffer,gt.byteOffset,gt.byteLength),L*m)}s.queue.writeBuffer(G,0,E),s.queue.writeBuffer(c,0,E.subarray(f*m,(f+1)*m))}if(b=f+1,x=u.generations.slice(0,f+1),h=x[f],g.bitsPerCell===p.bitsPerCell){let E=s.createCommandEncoder();E.copyBufferToBuffer(G,f*m,c,0,m),s.queue.submit([E.finish()])}let Be=y.splice(d).map(E=>E.filename);En(Be)}if(Qt(),me(),$=-1,M)q=!0;else{let l=s.createCommandEncoder();W(l),s.queue.submit([l.finish()]),K()}ae();break}case"stepForward":{if(Nr(),t.count===1){if(B&&lt()&&ee()&&(b>=C&&V(),Z(h)),st(),oe++,B&&ee()&&(b>=C&&V(),Z(h)),$=-1,M)q=!0;else{let r=s.createCommandEncoder();W(r),s.queue.submit([r.finish()]),K()}ae()}else self.postMessage({type:"stepping",active:!0}),B&&lt()&&ee()&&(b>=C&&V(),Z(h)),wt=I,Je=k,D=h+t.count,I=!0,k=0,O=0;break}case"cancelStepping":{if(D>=0){if(D=-1,I=wt,k=Je,j=0,X=0,$=-1,M)q=!0;else{let r=s.createCommandEncoder();W(r),s.queue.submit([r.finish()]),K()}ae(),self.postMessage({type:"stepping",active:!1})}break}case"updateChunkCodec":{let r=y.find(n=>n.filename===t.filename);r&&(r.codec=t.codec,r.storedBytes=t.storedBytes,r.gridFormat=t.gridFormat,H.chunks=[...y],me());break}case"getUncompressedChunks":{let r=y.filter(n=>n.codec===dt).map(n=>({filename:n.filename,rawBytes:n.uncompressedBytes,blockCount:n.blockCount,cols:S,rows:_,rawGridFormat:n.gridFormat,storageGridFormat:Pe(yt(Re.tribes.length))}));self.postMessage({type:"uncompressedChunks",chunks:r});break}}};
