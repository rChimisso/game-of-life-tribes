...
| Neighbor count range          |                               $0$ to $8$ | Moore neighborhood has exactly $8$ neighbors.                         |
...
| Max recording frame           |                           $1\text{ GiB}$ | Keeps individual recorded frames bounded.                             |
| Recording chunk cap           |                         $256\text{ MiB}$ | Preferred chunk size before frame-size fallback.                      |
...
| Pending OPFS write budget     |                         $512\text{ MiB}$ | Limits raw unsaved recording pressure.                                |
| Pending compression budget    |                           $1\text{ GiB}$ | Limits queued compression pressure.                                   |
| Download chunk-mode threshold |                           $2\text{ GiB}$ | Avoids very large in-memory normal exports.                           |
| Snapshot streaming threshold  |                         $256\text{ MiB}$ | Switches large snapshots to streaming.                                |
| Snapshot repack block         |                          $64\text{ MiB}$ | Bounded block size during streaming snapshot work.                    |
| MP4 max dimension             |                       $4\,096\text{ px}$ | Keeps video output inside practical encoder limits.                   |
| MP4 small-grid reference      |                       $1\,080\text{ px}$ | Upscales small grids to more useful video size.                       |
