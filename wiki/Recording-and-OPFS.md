...
Chunk capacity is chosen from a preferred 256 MiB chunk buffer cap, or one frame when a single frame is larger than that cap:

```text
targetChunkBytes = frameBytes < 256 MiB ? min(256 MiB, maxRecordingBytes) : frameBytes
chunkFrameCapacity = floor(targetChunkBytes / frameBytes)
```

## Compression

Raw chunks use the `raw-packed` codec until compressed. Compression workers use raw deflate and then update chunk metadata to `deflate-raw` when stored bytes are ready.
