...
All $3$ active-fire states count equally as burning neighbors. Vegetation resistance determines both whether ignition occurs and its initial intensity:
...
![Wildfire state transitions](mermaid/wildfire-state-transitions.svg)
...
- **Maximum burn rate**: largest positive $1$-generation loss of `Grass`, `Bush`, and `Tree`, divided by initial fuel.
...
![Wildfire outcome distribution across all sampled densities](../analysis/wildfire_res/plots/01_total_burn_boxplot.png)

The detailed sweep shows a sharp crossover around $90\%$:

| Density | Median burned | $\mathbf{Q}_{25}$ | $\mathbf{Q}_{75}$ |                IQR |
| ------: | ------------: | ----------------: | ----------------: | -----------------: |
...
![Wildfire burn-fraction IQR](../analysis/wildfire_res/plots/01_total_burn_iqr.png)

Rare fadeouts still persist above the central crossover. At $91\%$, $3$ of $30$ runs burn less than $1\%$, while $26$ burn more than $50\%$. At $93\%$, $1$ run burns $1.234\%$ and the other $29$ exceed $50\%$.

![Wildfire regime-change highlight](../analysis/wildfire_res/plots/06_regime_change_highlight_85_to_95.png)
...
![Burn fraction by vegetation type](../analysis/wildfire_res/plots/02_burn_fraction_by_vegetation_type.png)
...
Together, these results distinguish $2$ effects: near the crossover, uncertain connectivity produces long, variable exploration of the fuel network; above it, broader fronts consume fuel more intensely and finish sooner.

![Wildfire duration, peak, time-to-peak, and burn-rate metrics](../analysis/wildfire_res/plots/03_fire_metrics.png)
...
| ------------------------- | ---: | ---------: | -------: |
| Nearest $\mathrm{p}_{10}$ |  $7$ |  $0.678\%$ |    $141$ |
| Nearest $\mathrm{p}_{50}$ | $28$ | $27.324\%$ |    $587$ |
| Nearest $\mathrm{p}_{90}$ |  $5$ | $43.984\%$ |    $904$ |

![Wildfire trajectories by density](../analysis/wildfire_res/plots/04_cumulative_burn_spaghetti_by_density.png)

![Representative Wildfire transition runs](../analysis/wildfire_res/plots/05_transition_runs_90.png)

### Transition Runs In Motion

The $3$ recordings below use the same $90\%$ density and correspond to the representative runs in the table and figure above. Together they show how randomized initial vegetation layouts produce rapid fadeout, intermediate spread, or a large burn even though the subsequent Wildfire evolution is deterministic.

|                                                                                       Low outcome – Run $\mathbf{7}$                                                                                       |                                                                                      Median outcome – Run $\mathbf{28}$                                                                                       |                                                                                       High outcome – Run $\mathbf{5}$                                                                                       |
| :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| <video controls muted playsinline preload="metadata" width="100%"><source src="../analysis/wildfire_res/videos/wildfire-90-run-07-low.mp4" type="video/mp4">Your browser does not support embedded video.</video> | <video controls muted playsinline preload="metadata" width="100%"><source src="../analysis/wildfire_res/videos/wildfire-90-run-28-median.mp4" type="video/mp4">Your browser does not support embedded video.</video> | <video controls muted playsinline preload="metadata" width="100%"><source src="../analysis/wildfire_res/videos/wildfire-90-run-05-high.mp4" type="video/mp4">Your browser does not support embedded video.</video> |
...
The $50\%$ run is intentionally brief: its $4$-generation fadeout is representative of a regime in which fire cannot find a connected path through the fuel. The $100\%$ run shows the contrasting broad fire front and the survival of some resistant vegetation.

|                                                                                  Typical $\mathbf{50\%}$ – Run $\mathbf{24}$                                                                                   |                                                                                   Typical $\mathbf{100\%}$ – Run $\mathbf{6}$                                                                                   |
| :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| <video controls muted playsinline preload="metadata" width="100%"><source src="../analysis/wildfire_res/videos/wildfire-50-run-24-typical.mp4" type="video/mp4">Your browser does not support embedded video.</video> | <video controls muted playsinline preload="metadata" width="100%"><source src="../analysis/wildfire_res/videos/wildfire-100-run-06-typical.mp4" type="video/mp4">Your browser does not support embedded video.</video> |
...
<video controls muted playsinline preload="metadata" width="100%">
  <source src="../analysis/wildfire_res/videos/wildfire-89-run-05-long-lived.mp4" type="video/mp4">
